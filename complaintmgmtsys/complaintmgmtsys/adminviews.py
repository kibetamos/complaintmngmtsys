...
#     }
#     print(teams.count());
#     return render(request,'superuser/admindashboard.html',context)
...
def ADMINHOME(request):
    ...
    for complaint in complaints:
        print(f"Complaint: {complaint.complaint_number}, Resolved By: {complaint.resolved_by}")
...
def staff_dashboard(request):
    ...
    user_count = UserReg.objects.count()  # FIXED: added ()
...
def MANAGE_TEAM(request):
    ...
    context = {'teams': teams}
    return render(request, 'admin/manage_team.html', context)
...
def UPDATE_TEAM_DETAILS(request):
    if request.method == 'POST':
        team_id = request.POST.get('team_id')
        name = request.POST.get('name')  # Ensure this matches the form field
        description = request.POST.get('description')

        # Use get_object_or_404 to handle missing team gracefully
        team = get_object_or_404(Team, id=team_id)
        team.name = name  # Update the team name
        team.description = description  # Update the description
        team.save()

        messages.success(request, "Your team detail has been updated successfully")
        return redirect('manage_team')
